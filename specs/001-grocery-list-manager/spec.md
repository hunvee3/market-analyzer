...
- Q: Should the item sub-form be inline inside the list modal or open in a separate dialog? → A: Separate dialog — clicking "+ Add item" or the edit icon opens a focused MUI Dialog above the list modal. The dialog closes when the item is confirmed or cancelled.
